...
class GameLevel2Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Override swap count to 15 for level 2
        swapCountViewModel.swapCountLiveData.value = 15
    ...
    private fun clearSymbols(startRow: Int, startCol: Int, endRow: Int, endCol: Int) {
        for (row in startRow..endRow) {
            for (col in startCol..endCol) {
                val currentSymbol = symbol[row * noOfBlock + col].tag as Int
                if (currentSymbol == R.drawable.sm2) {
                    symCount1++
                    updateSymCountTextView(symCount1, R.id.symCount1)
                } else if (currentSymbol == R.drawable.sm5) {
                    symCount2++
    ...
    private fun navigateToScoreActivity() {
        ...
        if (score > highestScore) {
            // Check if both symCount1 and symCount2 are less than 10
            if (symCount1 < 10 || symCount2 < 10) {
